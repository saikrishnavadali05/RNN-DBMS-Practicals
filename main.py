...
from golden_ai.metrics import print_metrics_block
...
def run_demo() -> None:
    print()
    print("Gold price demo (normalized series -> models -> INR per 10g).")
    print("Read top to bottom; each block explains one step.")

    series, n_lo, n_hi, p_lo, p_hi, provenance = load_lab_series_from_duckdb()
    _banner("1) Load data from DuckDB")
    print(f"  Source: {provenance}")
    print(
        f"  Normalized scale used in training: [{n_lo:g}, {n_hi:g}]"
        f"  maps to  INR per 10g: [{p_lo:,.2f}, {p_hi:,.2f}]"
    )

    X, y = window(series, WINDOW)
    split = train_test_split_index(len(y))
    _banner("2) Explore the series (before fitting models)")
    print_exploration_report(series, X, y, split)
    X_train, y_train = X[:split], y[:split]
    X_test, y_test = X[split:], y[split:]

    _banner("3) Test-set scores (lower error is better unless noted)")
    print("  Strategy: last window value = naive next-step guess (baseline).")
    print_metrics_block("Plain baseline", test_metrics_plain(X_test, y_test))

    reg = fit_ridge(X_train, y_train)
    print()
    print("  Strategy: Ridge regression on the WINDOW past values.")
    print_metrics_block("Ridge", test_metrics_ml(reg, X_test, y_test))

    X_train_3d = X_train.reshape(-1, WINDOW, 1)
    model = fit_keras_model("rnn", X_train_3d, y_train, epochs=min(EPOCHS, 30))
    print()
    print("  Strategy: small recurrent network (SimpleRNN) in Keras.")
    print_metrics_block("SimpleRNN", test_metrics_keras(model, X_test, y_test))

    _banner("4) Multi-step roll forecast (Ridge) and sample actual vs predicted")
